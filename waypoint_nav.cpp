...
void land();														   //landing
void move(float lx, float ly, float lz, float ax, float ay, float az, int moveTime); // publishing command velocity
 ...
 * MOVE TO POINT
 **/
void move(float lx, float ly, float lz, float ax, float ay, float az, int moveTime)
{
	//ros::Rate loop_rate(10);
	//ros::spinOnce(); //if this function is not mentioned the function will stay in the buffer and cannot be able to publish
	//loop_rate.sleep();
